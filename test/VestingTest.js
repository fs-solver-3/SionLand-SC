...
//         cliffDuration = 4;
//         duration = 10;
//         VestingInstance = await ethers.getContractFactory("SionVesting");
//     });
...
//                 true
//             ),
//             "SionVesting: cliff is longer than duration"
//         );
//     });
...
//                 true
//             ),
//             "SionVesting: beneficiary is the zero address"
//         );
//     });
...
//                 true
//             ),
//             "SionVesting: final time is before current time"
//         );
//     });
...
//             cliffDuration = 4;
//             duration = 10;
//             VestingContract = await VestingInstance.deploy(
//                 sionTokenContract.address,
//                 beneficiary,
//                 start,
//                 cliffDuration,
...
//             await expectRevert(
//                 VestingContract.connect(account1).release(),
...
//             expect(await VestingContract.balanceOf()).to.be.equal(amountLockedToken);
//             await expectRevert(
//                 VestingContract.release(),
//                 "SionVesting: no tokens are due"
//             );
//         });
...
//             expect(await sionTokenContract.balanceOf(beneficiary)).to.be.equal(
//                 releasedAmount
//             );
//             expect(await VestingContract.balanceOf()).to.be.equal(
//                 amountLockedToken - releasedAmount
//             );
//         });
...
//                 amountLockedToken
//             );
//             expect(await VestingContract.balanceOf()).to.be.equal(0);
//         });
...
//             await expectRevert(
//                 VestingContract.connect(account1).revoke(),
//                 "Ownable: caller is not the owner"
//             );
//             await VestingContract.revoke();
//             expect(await VestingContract.revoked(beneficiary)).to.equal(true);
//             expect(
//                 await sionTokenContract.balanceOf(VestingContract.address)
...
//             await expectRevert(
//                 VestingContract.revoke(),
//                 "SionVesting: token already revoked"
//             );
//         });
//     });
// });